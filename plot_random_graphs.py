...
csv_folder = "benchmarks/random_graphs"
...
ordered_sizes = (
    df_all.groupby("graph_size")["mean_runtime"]
    .min()
    .sort_values()
    .index
)

# Print the sum of runtimes for each algorithm and the ordered graph sizes
print("Total Runtime per Algorithm:")
...
labels = [f"{n}, {m}" for n, m in ordered_sizes]

plt.xticks(x + width * (len(algorithms)-1)/2, labels)
plt.xlabel(r"Graph Size $(|V|, |E|)$")
