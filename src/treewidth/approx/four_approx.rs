...
pub fn approx_treewidth(graph: &graph::Graph) -> usize {
    let graph::Graph::AdjList(g) = graph else {
...
pub fn two_third_vertex_separator(
    ...
    for &orig_v in subset_vec.iter() {
        ...
        add_arc(&mut base_edges, &mut base_cap, &mut base_seen, vin(v), vout(v), 1);
        ...
        for &orig_u in neighbors {
            if subset.contains(&orig_u) {
                ...
                add_arc(&mut base_edges, &mut base_cap, &mut base_seen, vout(v), vin(u), inf);
            }
        }
    }
    ...
    for w1_refs in w_vec.iter().combinations(size_w1) {
        ...
        let mut w1_seen = base_seen.clone();

        // Add W1 clique.
        for (&u, &v) in w1.iter().tuple_combinations() {
            let u = node_map[&u];
            let v = node_map[&v];
            add_arc(&mut w1_edges, &mut w1_cap, &mut w1_seen, vout(u), vin(v), inf);
            add_arc(&mut w1_edges, &mut w1_cap, &mut w1_seen, vout(v), vin(u), inf);
        }

        // Connect source to W1.
        for &u in &w1 {
            let u = node_map[&u];
            add_arc(&mut w1_edges, &mut w1_cap, &mut w1_seen, src_out, vin(u), inf);
        }
        ...
        for w2_refs in remaining.iter().combinations(size_w2) {
            ...
            let mut w2_seen = w1_seen.clone();

            // Add W2 clique.
            for (&u, &v) in w2.iter().tuple_combinations() {
                let u = node_map[&u];
                let v = node_map[&v];
                add_arc(&mut w2_edges, &mut w2_cap, &mut w2_seen, vout(u), vin(v), inf);
                add_arc(&mut w2_edges, &mut w2_cap, &mut w2_seen, vout(v), vin(u), inf);
            }

            // Connect W2 to sink.
            for &u in &w2 {
                let u = node_map[&u];
                add_arc(&mut w2_edges, &mut w2_cap, &mut w2_seen, vout(u), sink_in, inf);
            }
...
fn add_arc(edges: &mut Vec<(usize, usize)>, capacities: &mut Vec<usize>, seen: &mut HashSet<(usize, usize)>, from: usize, to: usize, cap: usize) {
    // TODO: From what i have tested, seen makes it slower, but I should do more testing to be
    // sure.
