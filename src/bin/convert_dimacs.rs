...
fn graph_from_dimacs_file(file_path: &str) -> io::Result<Graph> {
    ...
    let n = loop {
        ...
        if line.starts_with('c') {
            ...
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Could not load graph, because the file must contain a 'p' line before any 'e' line"));

        } else {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Could not load graph, because the file does not contain a line starting with 'p'"));
        }
    };
    ...
    while let Some(line) = reader.next() {
        ...
        if line.starts_with('c') {
            ...
            let u = line_iter.nth(1)
                .expect(err_msg)
                .parse::<usize>()
                .expect(err_msg) - 1;

            let v = line_iter.next()
                .expect(err_msg)
                .parse::<usize>()
                .expect(err_msg) - 1;
...
fn main() -> io::Result<()> {
    ...
    for entry in fs::read_dir(dir_path)? {
        ...
        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("col") {
            ...
            writeln!(&file, "{} {}", path.file_stem().unwrap().to_string_lossy(), g6)?;
        }
    }
